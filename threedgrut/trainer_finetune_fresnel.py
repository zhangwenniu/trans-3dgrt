...
class Trainer:
    ...
    def train_color_loss(self, gpu_batch):
        ...
        # 调用RefractTracer的forward函数，获取返回的字典
        ret_dict = ray_tracer(masked_rays_o, masked_rays_d, self.renderer.sdf_network)
        
        # 如果ret_dict不为空，则获取折射和反射的射线信息
        if ret_dict:
            refract_rays_o = ret_dict['refract_rays_o']
            refract_rays_d = ret_dict['refract_rays_d']
            reflect_rays_o = ret_dict['reflect_rays_o']
            reflect_rays_d = ret_dict['reflect_rays_d']
            refract_rate = ret_dict['refract_rate']
            reflect_rate = ret_dict['reflect_rate']
            refract_indices = ret_dict['refract_indices']
        else:
            return None
        
        # ========= Reflect =============== #
        # 将折射和反射的射线变换回原始坐标系
        refract_rays_o = refract_rays_o * scale_mat[0, 0] + scale_mat[:3, 3]
        reflect_rays_o = reflect_rays_o * scale_mat[0, 0] + scale_mat[:3, 3]
        
        # 注意：方向向量只需要归一化，不需要进行坐标变换
        refract_rays_d = F.normalize(refract_rays_d, p=2, dim=-1)
        reflect_rays_d = F.normalize(reflect_rays_d, p=2, dim=-1)
        
        # 将射线变形为(1,1,-1,3)的形状
        refract_rays_o = refract_rays_o.view(1, 1, -1, 3)
        refract_rays_d = refract_rays_d.view(1, 1, -1, 3)
        reflect_rays_o = reflect_rays_o.view(1, 1, -1, 3)
        reflect_rays_d = reflect_rays_d.view(1, 1, -1, 3)

        selected_rgb_gt = masked_rgb_gt[refract_indices].view(1, 1, -1, 3)
        
        
        # 更新折射射线的gpu_batch
        refract_gpu_batch = copy.deepcopy(gpu_batch)
        refract_gpu_batch.rays_ori = refract_rays_o
        refract_gpu_batch.rays_dir = refract_rays_d
        refract_gpu_batch.rgb_gt = selected_rgb_gt
        
        # 更新反射射线的gpu_batch
        reflect_gpu_batch = copy.deepcopy(gpu_batch)
        reflect_gpu_batch.rays_ori = reflect_rays_o
        reflect_gpu_batch.rays_dir = reflect_rays_d
        reflect_gpu_batch.rgb_gt = selected_rgb_gt
        ...
        refract_rgb = refract_outputs["pred_rgb"]
        reflect_rgb = reflect_outputs["pred_rgb"]
        
        # 计算最终颜色
        # final_rgb = refract_rgb * refract_rate.view(1, 1, -1, 1) + reflect_rgb * reflect_rate.view(1, 1, -1, 1)
        
        final_rgb = refract_rgb * refract_rate.view(1, 1, -1, 1)
        
        color_loss = ((final_rgb - selected_rgb_gt) ** 2).mean()
        
        return color_loss * 0.1
