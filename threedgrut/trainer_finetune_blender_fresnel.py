...
from threedgrut.trace_refract_fresnel import RefractTracer
...
class Trainer:
    def __init__(
        self, model, sdf_checkpoint_path, conf, global_step, out_dir
    ) -> None:
        """
        训练器类的初始化方法，简化版本
        ...
        self.init_from_neus(sdf_checkpoint_path)
    ...
    @classmethod
    def from_checkpoint(cls, sdf_checkpoint_path, gaussian_checkpoint_path, out_dir, data_dir, experiment_name):
        """
        从检查点加载模型和配置，简化版本
        ...
        """
        # 加载检查点文件
        logger.info(f"从检查点加载Gaussian Splatting模型: {gaussian_checkpoint_path}")
        checkpoint = torch.load(gaussian_checkpoint_path)
        global_step = checkpoint["global_step"]

        # 从检查点获取配置信息
        # conf = checkpoint["config"]
        
        # 传入conf的原因是，3dgrt在blender数据集上面不容易收敛，采用3dgut做场景训练，使用3dgrt作为渲染器，支持光线追踪
        conf = checkpoint["config"]
        
        conf.path = data_dir
        conf.experiment_name = experiment_name
        # conf.render.method = "3dgrt"
        conf.dataset.type = "nerf_with_mask"
        ...
        return cls(
            model=model,
            sdf_checkpoint_path=sdf_checkpoint_path,
            conf=conf,
    ...
    def init_from_neus(self, sdf_checkpoint_path):
        ...
        checkpoint = torch.load(sdf_checkpoint_path, map_location=device)
        ...
        self.optimizer = torch.optim.Adam(params_to_train, lr=0.00001)
    ...
    def train_silhouette(self, gpu_batch):
        ...
        # 计算silhouette loss        
        silhouette_loss = torch.nn.functional.binary_cross_entropy(
            weights_sum.clip(1e-3, 1-1e-3).squeeze(), 
            (selected_mask > 0.5).float().squeeze()
        ) / rays_o.shape[0]
        ...
        return {
            ...
            'normal_smoothness_loss': normal_smoothness_loss
        }
    ...
    def train_color_loss(self, gpu_batch):
        """使用RayTracing类实现光线折射渲染，并根据射线类型合成最终颜色。
        
        参数:
            gpu_batch: 包含射线信息的批次数据
        返回:
            color_loss: 计算得到的颜色损失
        """
        # 获取射线原点和方向
        rays_o = gpu_batch.rays_ori
        rays_d = gpu_batch.rays_dir
        ...
        # 调用RefractTracer的forward函数，获取返回的字典
        ray_results = ray_tracer(masked_rays_o, masked_rays_d, self.renderer.sdf_network)
        
        if not ray_results:
            logger.warning("折射光线追踪无结果，跳过颜色损失计算")
            return None
        
        # 提取不同类型的掩码
        no_intersection_mask = ray_results['no_intersection_mask']
        total_reflection_mask = ray_results['total_reflection_mask']
        twice_refraction_mask = ray_results['twice_refraction_mask']
        
        # 提取各种射线信息
        reflect_rays_o = ray_results['reflect_rays_o']
        reflect_rays_d = ray_results['reflect_rays_d']
        refract_rays_o = ray_results['refract_rays_o']
        refract_rays_d = ray_results['refract_rays_d']
        reflect_rate = ray_results['reflect_rate']
        refract_rate = ray_results['refract_rate']
        
        # 初始化最终颜色
        final_rgb = torch.zeros_like(masked_rgb_gt)
        
        # 1. 处理没有交点的射线 - 使用原始方向的颜色
        if no_intersection_mask.any():
            original_rays_o = masked_rays_o[no_intersection_mask]
            original_rays_d = masked_rays_d[no_intersection_mask]
            
            # 变换回原始空间
            original_rays_o = original_rays_o * scale_mat[0, 0] + scale_mat[:3, 3]
            
            # 创建GPU batch
            original_gpu_batch = copy.deepcopy(gpu_batch)
            original_gpu_batch.rays_ori = original_rays_o.view(1, 1, -1, 3)
            original_gpu_batch.rays_dir = original_rays_d.view(1, 1, -1, 3)
            
            # 渲染原始方向
            with torch.no_grad():
                original_outputs = self.model(original_gpu_batch, train=False)
            
            # 将结果填充到最终颜色中
            final_rgb[no_intersection_mask] = original_outputs["pred_rgb"].view(-1, 3)
        
        # 2. 处理有交点的射线 - 添加反射颜色
        has_intersection_mask = ~no_intersection_mask
        if has_intersection_mask.any():
            # 变换射线回原始空间
            reflect_rays_o_valid = reflect_rays_o[has_intersection_mask]
            reflect_rays_d_valid = reflect_rays_d[has_intersection_mask]
            reflect_rate_valid = reflect_rate[has_intersection_mask]
            
            # 坐标变换
            reflect_rays_o_valid = reflect_rays_o_valid * scale_mat[0, 0] + scale_mat[:3, 3]
            
            # 创建GPU batch
            reflect_gpu_batch = copy.deepcopy(gpu_batch)
            reflect_gpu_batch.rays_ori = reflect_rays_o_valid.view(1, 1, -1, 3)
            reflect_gpu_batch.rays_dir = reflect_rays_d_valid.view(1, 1, -1, 3)
            
            # 渲染反射
            with torch.no_grad():
                reflect_outputs = self.model(reflect_gpu_batch, train=False)
            
            # 将反射结果填充到最终颜色中
            final_rgb[has_intersection_mask] = reflect_outputs["pred_rgb"].view(-1, 3) * reflect_rate_valid
        
        # 3. 处理两次折射的射线 - 添加折射颜色
        if twice_refraction_mask.any():
            # 变换射线回原始空间
            refract_rays_o_valid = refract_rays_o[twice_refraction_mask]
            refract_rays_d_valid = refract_rays_d[twice_refraction_mask]
            refract_rate_valid = refract_rate[twice_refraction_mask]
            
            # 坐标变换
            refract_rays_o_valid = refract_rays_o_valid * scale_mat[0, 0] + scale_mat[:3, 3]
            
            # 创建GPU batch
            refract_gpu_batch = copy.deepcopy(gpu_batch)
            refract_gpu_batch.rays_ori = refract_rays_o_valid.view(1, 1, -1, 3)
            refract_gpu_batch.rays_dir = refract_rays_d_valid.view(1, 1, -1, 3)
            
            # 渲染折射
            with torch.no_grad():
                refract_outputs = self.model(refract_gpu_batch, train=False)
            
            # 将折射结果加到最终颜色中
            indices_in_full = torch.where(twice_refraction_mask)[0]
            final_rgb[indices_in_full] += refract_outputs["pred_rgb"].view(-1, 3) * refract_rate_valid
        
        # 计算颜色损失
        color_loss = ((final_rgb - masked_rgb_gt) ** 2).mean()
        
        # 记录统计信息
        with torch.no_grad():
            num_no_intersection = no_intersection_mask.sum().item()
            num_total_reflection = total_reflection_mask.sum().item()
            num_twice_refraction = twice_refraction_mask.sum().item()
            
            logger.info(f"射线统计: 无交点={num_no_intersection}, 全反射={num_total_reflection}, 两次折射={num_twice_refraction}")
    ...
    def run_training(self):
        ...
        for epoch_idx in range(self.n_epochs):
            ...
            for batch in epoch_progress:
                ...
                loss += silhouette_losses['silhouette_loss'] * 0.1 + silhouette_losses['eikonal_loss'] * 0.1 + silhouette_losses['normal_smoothness_loss'] * 0.005
                # 应用折射光线追踪 - 现在返回gpu_batch和法线平滑度损失
                # color_loss = self.train_color_loss(gpu_batch)
                # if color_loss:
    ...
    def render_from_trace_refract_fresnel(self, gpu_batch, renderer, resolution_level=1):
        """使用RayTracing类实现光线折射渲染，并直接生成最终的渲染图像。
        ...
        参数:
            gpu_batch: 包含射线信息的批次数据
            renderer: 渲染器对象，包含SDF网络
            resolution_level: 下采样因子，用于加速测试渲染
            
        返回:
            rendered_image: 最终的渲染图像
        """
        # 获取射线原点和方向
        rays_o = gpu_batch.rays_ori
        rays_d = gpu_batch.rays_dir
        ...
        # 创建RefractTracer实例
        ray_tracer = RefractTracer(ior_air=n1, ior_object=n2, n_samples=128)
        
        # 扁平化射线以便处理
        rays_o_flat = rays_o.reshape(-1, 3)
        rays_d_flat = rays_d.reshape(-1, 3)
        mask_flat = mask.reshape(-1, 1) if mask is not None else None
        ...
        # 创建最终渲染图像的容器
        rendered_rgb = torch.zeros(batch_size, H, W, 3, device=rays_o.device)
        ...
        # 按批次处理所有射线
        for i in tqdm(range(0, len(rays_o_flat), iter_size), desc="Refractive Rendering", unit="batch"):
            end_idx = min(i + iter_size, len(rays_o_flat))
            batch_indices = torch.arange(i, end_idx, device=rays_o_flat.device)
            ...
            # 进行光线追踪
            ray_results = ray_tracer(batch_rays_o, batch_rays_d, renderer.sdf_network)
            
            # 初始化批次渲染结果
            batch_rgb = torch.zeros(len(batch_indices), 3, device=rays_o.device)
            
            # 1. 渲染所有射线的原始方向颜色
            # 变换回原始空间
            orig_rays_o = batch_rays_o * scale_mat[0, 0] + scale_mat[:3, 3]
            
            # 创建GPU batch
            orig_gpu_batch = copy.deepcopy(gpu_batch)
            orig_gpu_batch.rays_ori = orig_rays_o.reshape(1, 1, -1, 3)
            orig_gpu_batch.rays_dir = batch_rays_d.reshape(1, 1, -1, 3)
            
            # 渲染原始方向
            with torch.no_grad():
                outputs = self.model(orig_gpu_batch, train=False)
            
            # 获取原始方向的颜色
            original_rgb = outputs["pred_rgb"].reshape(-1, 3)
            
            # 设置所有射线的初始颜色为原始方向的颜色
            batch_rgb = original_rgb.clone()
            ...
            if ray_results:
                ...
                if has_intersection_mask.any():
                    # 获取有交点的射线索引
                    valid_indices = torch.where(has_intersection_mask)[0]
                    
                    # 处理反射部分
                    refl_rays_o = reflect_rays_o[valid_indices]
                    refl_rays_d = reflect_rays_d[valid_indices]
                    refl_rate = reflect_rate[valid_indices]
                    
                    # 变换回原始空间
                    refl_rays_o = refl_rays_o * scale_mat[0, 0] + scale_mat[:3, 3]
                    
                    # 创建GPU batch
                    refl_gpu_batch = copy.deepcopy(gpu_batch)
                    refl_gpu_batch.rays_ori = refl_rays_o.reshape(1, 1, -1, 3)
                    refl_gpu_batch.rays_dir = refl_rays_d.reshape(1, 1, -1, 3)
                    
                    # 渲染反射
                    with torch.no_grad():
                        outputs = self.model(refl_gpu_batch, train=False)
                    
                    # 获取反射颜色
                    reflect_rgb = outputs["pred_rgb"].reshape(-1, 3)
                    
                    # 更新颜色：有交点的射线添加反射颜色
                    batch_rgb[valid_indices] = reflect_rgb * refl_rate
                
                # 3. 处理两次折射的射线 - 添加折射颜色
                if twice_refraction_mask.any():
                    # 获取两次折射的射线索引
                    refract_indices = torch.where(twice_refraction_mask)[0]
                    
                    # 处理折射部分
                    refr_rays_o = refract_rays_o[refract_indices]
                    refr_rays_d = refract_rays_d[refract_indices]
                    refr_rate = refract_rate[refract_indices]
                    
                    # 变换回原始空间
                    refr_rays_o = refr_rays_o * scale_mat[0, 0] + scale_mat[:3, 3]
                    
                    # 创建GPU batch
                    refr_gpu_batch = copy.deepcopy(gpu_batch)
                    refr_gpu_batch.rays_ori = refr_rays_o.reshape(1, 1, -1, 3)
                    refr_gpu_batch.rays_dir = refr_rays_d.reshape(1, 1, -1, 3)
                    
                    # 渲染折射
                    with torch.no_grad():
                        outputs = self.model(refr_gpu_batch, train=False)
                    
                    # 获取折射颜色
                    refract_rgb = outputs["pred_rgb"].reshape(-1, 3)
                    
                    # 更新颜色：有两次折射的射线添加折射颜色
                    batch_rgb[refract_indices] += refract_rgb * refr_rate
            
            # 将当前批次的结果存入最终渲染图像
            rendered_rgb.reshape(-1, 3)[batch_indices] = batch_rgb
            
            # 清空CUDA缓存
            torch.cuda.empty_cache()
        
        # 重新整形为正确的尺寸
        rendered_rgb = rendered_rgb.reshape(batch_size, H, W, 3)
        
        # 输出一些统计信息
        logger.info(f"渲染完成: 图像尺寸 = {H}x{W}")
    ...
    def save_visualization(self, iteration, batch_idx=-1, resolution_level=1):
        ...
        try: 
            ...
            # 步骤3: 直接渲染带折射效果的图像
            rendered_rgb = self.render_from_trace_refract_fresnel(gpu_batch, self.renderer, resolution_level)
            ...
            # 只处理批次中的第一个图像
            pred_img = rendered_rgb[0].detach().cpu().numpy()  # [H, W, 3]
            ...
            if torch.sum(current_mask) > 0:
                # 使用布尔索引来选择掩码区域的像素
                pred_masked = rendered_rgb[0].detach().cpu()[current_mask.squeeze(-1)]  # 选择掩码为True的像素
            ...
            if torch.sum(non_mask) > 0:
                pred_non_masked = rendered_rgb[0].detach().cpu()[non_mask]  # 选择掩码为False的像素
            ...
            # 3. 全图的PSNR
            full_mse = torch.mean(torch.square(rendered_rgb[0].detach().cpu() - gt_rgb[0]))
            ...
            ax1.imshow(pred_img)
            ax1.set_title('Rendered Image with Refraction', fontsize=14)
            ax1.axis('off')
            ...
            # 保存图像
            plt.savefig(os.path.join(vis_dir, f'iter_{iteration:06d}_batch_{batch_idx:02d}.png'), dpi=150)
            plt.close()
