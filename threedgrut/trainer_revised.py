...
class Trainer3DGRUT:
    ...
    def get_losses(
        ...
        rgb_gt = gpu_batch.rgb_gt
        rgb_pred = outputs["pred_rgb"]
        # print("gpu_mask.shape:", gpu_batch.mask.shape)
        # print("gpu_mask.max:", gpu_batch.mask.max())
        # print("gpu_mask.min:", gpu_batch.mask.min())
        mask = gpu_batch.mask > 0.5
        # print("gpu_mask.sum:", mask.sum())
        # print("rgb_gt.shape:", rgb_gt.shape)
        # print("rgb_pred.shape:", rgb_pred.shape)
        
        if mask.sum() > 0:
            self.conf.loss.use_ssim = False
            self.conf.loss.lambda_l1 = 1.0
        ...
        if self.conf.loss.use_l1:
            with torch.cuda.nvtx.range(f"loss-l1"):
                # 不需要reshape，直接使用原始形状
                # 首先确保掩码的维度与RGB图像匹配（在最后一个维度上扩展）
                mask_expanded = ~mask.expand(-1, -1, -1, 3)  # 变成 [1, 900, 1200, 3]
                abs_diff = torch.abs(rgb_pred - rgb_gt)
                masked_abs_diff = abs_diff[mask_expanded]                
                loss_l1 = masked_abs_diff.mean()
        ...
        return dict(total_loss=loss, l1_loss=loss_l1, l2_loss=torch.zeros(1, device=self.device), ssim_loss=torch.zeros(1, device=self.device))
    ...
    def run_train_pass(self, conf: DictConfig):
        ...
        for iter, batch in enumerate(self.train_dataloader):
            ...
            if self.global_step % 1000 == 0:
                ...
                cv2.imwrite(os.path.join(output_dir, f"img_pred_{self.global_step}.png"), concated_img)
