...
class Dataset:
    def __init__(self, conf):
        ...
        self.light_masks = torch.from_numpy(self.light_mask_np.astype(np.float32)).to(self.device) # [n_images, H, W, 3]
        self.masks  = torch.from_numpy(self.masks_np.astype(np.float32)).to(self.device)   # [n_images, H, W, 3]
        self.masks_bound = torch.from_numpy(self.masks_bound_np.astype(np.float32)).to(self.device)
        ...
        self.screen_point = torch.from_numpy(self.screen_point_np.astype(np.float32)).to(self.device)#[n_image, H*W, 3]
    ...
    def gen_random_rays_at(self, img_idx, batch_size):
        ...
        return torch.cat([rays_o.to(self.device), rays_v.to(self.device), ray_point, mask, valid_mask], dim=-1).cuda(), pixels_x, pixels_y
    ...
    def gen_rays_at(self, img_idx, resolution_level=1):
        ...
        # 只在索引mask时使用整数索引
        # print("dtype of masks: ", self.masks.dtype, "device of masks: ", self.masks.device)
        # print("dtype of pixels_y_idx: ", pixels_y_idx.dtype, "device of pixels_y_idx: ", pixels_y_idx.device)
        # print("dtype of pixels_x_idx: ", pixels_x_idx.dtype, "device of pixels_x_idx: ", pixels_x_idx.device)
        # print("dtype of img_idx: ", img_idx.dtype, "device of img_idx: ", img_idx.device)
        mask = self.masks[img_idx][(pixels_y_idx, pixels_x_idx)]
        return rays_o.transpose(0, 1), rays_v.transpose(0, 1), mask[..., :1].transpose(0, 1)
